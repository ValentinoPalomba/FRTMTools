...
private enum APKSizeBreakdownCalculator {
    static func calculate(from root: FileInfo) -> InstalledSizeMetrics {
        ...
        let binaries = max(Int(dexBytes / 1_048_576), 0)
        let frameworks = max(Int(nativeBytes / 1_048_576), 0)
        let resourcesBytes = max(totalBytes - dexBytes - nativeBytes, 0)
        let resources = max(Int(resourcesBytes / 1_048_576), 0)
        return InstalledSizeMetrics(total: total, binaries: binaries, frameworks: frameworks, resources: max(resources, 0))
    }
    ...
    private func inferABIs(from rootFile: FileInfo) -> [String] {
        var set: Set<String> = []
        collectABIs(in: rootFile, accumulator: &set)
        return set.sorted()
    }

    private func collectABIs(in node: FileInfo, accumulator: inout Set<String>) {
        guard let subItems = node.subItems else { return }

        if node.type == .directory, node.name == "lib" {
            for child in subItems where child.type == .directory {
                accumulator.insert(child.name)
            }
        }

        for child in subItems {
            collectABIs(in: child, accumulator: &accumulator)
        }
    }
}
